...
class AlumniController < ApplicationController
  ...
  def show
    Current.previous_path = "/users/#{String(@alumnus.user.id)}" unless Current.user.is_admin?
  end
  ...
  # GET /alumni/1/edit
  def edit
    Current.previous_path = "/alumni/#{String(@alumnus.id)}" unless Current.user.is_admin?
  end
